...
static uint8_t DeviceAT24CxxReadByte(uint16_t addr, uint8_t* readDat)
{
    ...
    {
        /*写入设备写地址*/
        CoreIICSendByte(DEVICE_AT24CXX_ADDR_W);
        /*等待设备响应*/
        ...
        /*写入目标地址高地址*/
        CoreIICSendByte((uint8_t)(addr >> 8));
    }
    else
    {
        /*发送器件地址,写数据*/
        CoreIICSendByte(DEVICE_AT24CXX_ADDR_W + (uint8_t)((addr / 256) << 1));
    }
    /*等待设备响应*/
    waitAckResult = CoreIICWaitACK();
    if (IIC_WAIT_ACK_FAIL == waitAckResult)
    ...
    /*写入目标地址高地址*/
    CoreIICSendByte((uint8_t)(addr % 256));
    /*等待设备响应*/
    waitAckResult = CoreIICWaitACK();
    if (IIC_WAIT_ACK_FAIL == waitAckResult)
    {
    ...
    CoreIICSendStart();
    /*写入设备读地址*/
    CoreIICSendByte(DEVICE_AT24CXX_ADDR_R);
    /*等待设备响应*/
    waitAckResult = CoreIICWaitACK();
    if (IIC_WAIT_ACK_FAIL == waitAckResult)
    {
