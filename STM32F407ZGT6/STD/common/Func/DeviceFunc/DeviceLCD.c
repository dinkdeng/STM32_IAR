...
//LCD的画笔颜色和背景色
//画笔颜色
u16 POINT_COLOR=LCD_DEFAULT_POINT_COLOR;
//背景色
u16 BACK_COLOR=LCD_DEFAULT_BACK_COLOR;
...
void LCD_Scan_Dir(u8 dir)
{
	...
	if(lcddev.id==0x9341||lcddev.id==0X6804|| \
		lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)
	{
		switch(dir)
		{
...
void LCD_Init(void)
{
	...
	else if(lcddev.id==0X6804||lcddev.id==0XC505)
	{
		...
		//数据保存时间清零
		FSMC_Bank1E->BWTR[6]&=~(0XF<<8);
		//地址建立时间(ADDSET)为10个HCLK =60ns
		FSMC_Bank1E->BWTR[6]|=10<<0;
		//数据保存时间(DATAST)为6ns*13个HCLK=78ns
	...
	else if(lcddev.id==0x9328)
	{
        ...
        LCD_WriteReg(0x0013,0x0000);
     	LCD_WriteReg(0x0007,0x0001);
       	DEVICE_LCD_WAIT_MS(LCD_ID9328_WAIT_CONFIG_TIME);
	...
	else if(lcddev.id==0x5408)
	{
		...
		//RAM Address Start – Partial Display 2
		LCD_WriteReg(0x84,0x0000);
		//RAM address End – Partail Display2
		LCD_WriteReg(0x85,0x0000);
		//Frame Cycle Control
	...
	else if(lcddev.id==0x4535)
	{
		...
		DEVICE_LCD_WAIT_MS(40);
		//16
		LCD_WriteReg(0X12,0X0012);
		DEVICE_LCD_WAIT_MS(40);
  		LCD_WriteReg(0X10,0X3420);
 		LCD_WriteReg(0X13,0X3038);
		DEVICE_LCD_WAIT_MS(70);
		LCD_WriteReg(0X30,0X0000);
		LCD_WriteReg(0X31,0X0402);
...
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode)
{
	...
	for(t=0;t<csize;t++)
	{
		...
		//调用1608字体
			...
			temp=LCD_FONT_1608[num][t];
		//调用2412字体
		else if(size==24)
			temp=LCD_FONT_2412[num][t];
		//没有的字库
		else
			return;
		for(t1=0;t1<8;t1++)
